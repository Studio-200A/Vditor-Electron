...
> **编写要求：** 读当前源码（通常 `src/renderer/app/app-composition.js`），列出本批次要迁出的 legacy 职责，格式统一为「`legacy 函数名` → 目标 `模块名`」；再列出上一批次已迁移、本批次不得重做的职责及其语义 owner。这是全文档最依赖源码核对的一节，不要凭记忆写。
...
- Linux 通过不代表 Windows/macOS 验证；平台限制继续记录在 `docs/04-CROSS-PLATFORM.md`。
- {{本批次专属边界 1}}。
- {{本批次专属边界 2}}。
...
- {{私有 DOM 假设项}}（例如：每个假设都有 focused adapter test，并更新 `docs/07-VDITOR-UPGRADE.md`）。
