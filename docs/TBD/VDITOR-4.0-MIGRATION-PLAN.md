...
- 重建或明确降级 SV 行号、空白符、滚动同步、divider、自动缩进、列表缩进、上下文菜单和剪贴板。滚动同步包含 0.2.6 的 source → preview 标题锚点配对（adapter `syncSplitScroll()`，依赖私有 source heading marker DOM）；4.0 的 textarea 源码面没有该结构，必须按 textarea/preview 映射重建或降级回比例同步。SV 尾部留白改用 textarea `paddingBottom`，不再依赖 3.11.3 的 `--editor-bottom`/`::after`；验证不增加逻辑行号、不破坏 typewriter、滚动或分栏比例。
