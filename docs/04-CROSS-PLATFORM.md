...
当前的开发与验证平台是 Linux。跨平台工作计划在已完成的 0.2.5 渲染器重构（已随 v0.2.5 发布）之后、且 Windows/macOS 测试环境就绪后开始。在此之前，Linux 测试可以防止平台无关的回归，但不得被报告为 Windows 或 macOS 验证。

已完成的 0.2.5 渲染器审查的 Linux 全量套件证据保留在 [`docs/ARCHIVED/15-0.2.5-EXECUTION-TRACKER.md`](ARCHIVED/15-0.2.5-EXECUTION-TRACKER.md) 中。在一次资源敏感的 Electron E2E 结果之后进行的 Linux 重试，仍只是 Linux 证据，不能满足任何 Windows 或 macOS 行。
...
0.2.5 起另有一条跨平台生效的安全边界：若用户请求的工作区根目录本身是符号链接，主进程在 `realpath()` 规范化之前先对该路径执行 `lstat()`，`scan()` 直接返回 `workspace-symbolic-link` 不可用结果，不开始扫描、不生成 revision 或部分结果（见 `src/main/services/resource-health-service.ts` 的 `isRequestedWorkspaceSymbolicLink`）；渲染器显示不可用覆盖层，警告用户出于安全考虑不支持该形态，并提示改为打开符号链接的目标目录作为工作区，也不得因 canonical 化而显示目标目录名称。两个平台的原生验证应覆盖该分支：以符号链接（Windows 含符号链接/junction 目录形态）作为工作区根时，资源健康整体不可用、显示警告，且不触发任何扫描或回收站能力。
