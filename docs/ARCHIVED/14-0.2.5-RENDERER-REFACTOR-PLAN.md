...
文件身份、保存基线、外部冲突、恢复、watcher 和已有目标 TOCTOU 边界的长期权威契约见 [`docs/06-FILE-SAFETY.md`](06-FILE-SAFETY.md)。本计划只补充 0.2.5 重构必须保留的结构约束，不复制或重新定义该安全模型。
...
- 保存正文的解码结果与传给安全写入器的 `expectedBytes` 必须来自同一次原始读取；安全写入器在最终替换前仍需保留字节复核。该复核与最终 `rename` 之间的平台原子性边界必须如实记录，不能把它描述成消除了所有 TOCTOU 风险；完整契约见 [`docs/06-FILE-SAFETY.md` §7](06-FILE-SAFETY.md#7-已知原子性边界已有目标的-toctou)。
- 异步保存、恢复、不可用状态处理和图片上传在每个会产生可见状态或写入的 await 边界后，必须复核 document 仍存在且其文件绑定、editor runtime 未被关闭、重绑或重建；关闭同一 document 的并发请求只能执行一次清理交易。recovery snapshot 丢弃失败时保留其 ID，以便后续清理而不产生孤立快照。

截至 2026-08-27，0.2.0 开发 tracker 批次 7 已完成 Linux 本地闭环。当前 `dev-0.2.0` 的提交基线为 `3e7eb15`，批次 7 的源码、测试和文档修复仍在工作树，提交哈希待按仓库流程创建；用户手动运行 `npm run check:all` 得到 149/149 单元测试和 109/109 Electron E2E 全部通过。该结果只构成 Linux 行为基线，不替代第 5.3 节和 [`docs/04-CROSS-PLATFORM.md` §9](04-CROSS-PLATFORM.md#9-020-批次-7-推迟的平台验证) 的实体机证据；开始 0.2.5 前仍需按阶段 0 重新建立稳定 tag/分支基线。
...
0.2.5 的完整开发和验收继续以 Linux 为主，Windows/macOS 的实体机回归留待后续专用阶段。重构不得把 Linux 假设固化到 renderer；平台边界、路径处理和测试证据统一遵循 [`docs/04-CROSS-PLATFORM.md`](04-CROSS-PLATFORM.md)。
...
│  ├─ toolbar-controller.ts        # 当前支持的显示/隐藏工具栏
...
- [ ] 保存 0.2.0 六种应用主题、三种编辑模式和当前支持的两种工具栏状态（显示/隐藏）的基准截图，供人工对比，不做脆弱的全页面像素测试；主题内容与应用壳层边界单独标注。
...
阶段 4 完成文档命令、identity、安全写入参数、快照 DTO 和标签表现的边界。若保存交易需要取得 Vditor 当前正文、自动保存 timer、恢复正文注入、关闭时 runtime 释放，或外部变化需要更新 editor-owned UI，这些实际 runtime 协调应由阶段 5 的 EditorController 收口；不得因此复制或绕过阶段 4 的 DocumentController 安全命令。
...
- 接管阶段 4 文档命令所需的当前正文读取、自动保存 timer、保存交易调用、关闭时 runtime 释放、恢复正文注入与 editor-owned UI 协调；DocumentController 仍是保存、关闭、外部变化和恢复的安全语义入口。
...
负责当前支持的显示/隐藏工具栏模式、挂载目标和工具按钮过滤。它依赖 EditorController 的窄接口，不直接管理文档保存或设置窗口。浮动工具栏不属于 0.2.5 重构范围，不得因迁移新增。
...
- 将持久化文件按语义拆分：现有平台配置目录中的 `config.toml` 只保存用户明确选择的偏好与默认值；同目录新增版本化 `state.json`，保存需要跨启动保留的应用状态，例如最近文件与目录、打开标签与活动标签、工作区与每个工作区的目录展开状态、最近确认的对话框目录及窗口/UI 恢复状态。两者都是受支持、需可靠写入的持久化数据，只是拥有独立 schema、生命周期与重置边界。该目录由 `src/main/app-paths.ts` 在 Linux、Windows 和 macOS 上统一解析；不得为某个平台使用不同文件名或额外目录。
- 主进程必须以显式 schema/白名单读写 `state.json`，对未知版本、损坏 JSON 或无效字段安全降级为默认状态且不阻止启动；写入须保持当前持久化的原子性、队列和错误语义，不能把状态写失败静默视为成功。首次升级读取旧 `config.toml` 中的状态字段并一次性迁移到 `state.json`；迁移完成后 `config.toml` 不再写入这些字段。迁移必须幂等，不能覆盖已有有效 `state.json`，且不得把恢复快照或运行时句柄写入任一文件。
- 调整 IPC 与 renderer contracts，使设置保存/重置只影响 `config.toml`，会话、recent、workspace 与窗口状态使用窄 state API；不得以通用 JSON 文件读写 API 暴露给 renderer。
...
其他缺陷进入 `docs/00-ISSUES.md`，留到 0.3.0 或后续版本。

## 29. 文档要求

- [x] 新增 `docs/02-RENDERER-ARCHITECTURE.md`，说明模块、状态所有权、依赖方向和启动流程。
- [x] README 只需更新开发构建命令，不必向普通用户解释内部目录。
- [x] 更新 `docs/07-VDITOR-UPGRADE.md`，说明 EditorController 与 adapter 的边界。
- [x] 在贡献说明中记录新增 renderer 模块的放置原则。
- [x] 0.2.5 changelog 明确说明这是内部架构升级，用户功能基本保持一致。
...
- [ ] 不存在导出到 `window`、可被任意模块直接修改的全局 `state` 对象，或可绕过领域状态规则的公共 patch 入口。`app-composition.js` IIFE 内仅供既有接线使用的私有 `state` / `store` 投影不等同于此类全局状态；其可写路径仍须收敛到具名领域命令或已记录的兼容协调。
- [ ] `main.ts` 只负责组合和启动，没有重新成为业务逻辑仓库。
- [ ] `app/app-composition.js` 按 [`09-DEV-NOTE.md` 的“模块化重构边界`](09-DEV-NOTE.md#模块化重构边界)保留 controller 实例化、窄依赖注入、短委托和命名跨域协调；不以行数或函数数量判定失败。只有剩余函数拥有独立领域状态、资源生命周期、安全/IPC 边界，或可脱离 composition 闭包独立测试时，才应提取为新的领域模块。
...
- [ ] 已迁移的职责域不存在未解释的双重监听、旧新实现并行、延迟补偿写入或跨集合下标拼接；保留的 listener、timer 和 observer 均有明确 owner 与 cleanup。组合层可为该 owner 注入接线回调或注册入口，但不得成为第二个资源或领域状态 owner。
- [ ] preload、设置、标签页和 Vditor 实际用法具备有效类型；控制器只接收所需 bridge 能力，已迁移 channel 不保留宽泛契约，领域模块没有泛滥的 `any`。

## 32. 行为完成标准

- [ ] 0.2.0 的核心行为清单全部通过。
- [ ] 三种编辑模式和当前支持的两种工具栏状态（显示/隐藏）行为保持一致。
