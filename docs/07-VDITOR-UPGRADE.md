...
- `wordWrap` 通过 adapter 的 `applyWordWrap()` 标记 Vditor 3.11.3 的 SV 编辑根和 WYSIWYG/IR `.vditor-reset` 滚动根，再由应用 CSS 切换软换行；关闭换行时 WYSIWYG/IR 的段落宽度由滚动根的对称 margin 形成居中 viewport，preview 与代码块保持自身布局。升级时须核对三种模式的实际编辑滚动根、关闭换行后的水平滚动、40–100% 段落宽度的独立效果、SV 行号与自绘光标的水平滚动几何，并同步 adapter facade 契约及真实 Electron 回归。
...
- Desktop 的 SV divider 由 adapter 插入 Vditor 3.11.3 私有 `.vditor-content`，并以 source 与 preview 的实际 display 状态报告 source-only、preview-only、both 语义。编辑器重建交易用同一语义按 tab 捕获并恢复 pane 布局：source-only 经新实例 `preview.mode: 'editor'` 初始化，preview-only 经 `restorePreviewOnly()` 触发 Vditor 自身 Preview toolbar action（3.11.3 没有公开的 preview-only setter），不把 tab 内布局写成全局设置。升级时须验证 Vditor 异步 pane 替换后 divider 不重复、两单栏均隐藏 divider、both 中可拖动且关闭/重建不保留 window pointer listener，以及 both/source-only/preview-only 三种布局在初始化设置重建后保持不变；若上游提供公开的 pane 布局 setter，应替换 Preview toolbar action 模拟。
- Desktop 的 SV 行号、空白符 canvas、滚动同步、列表缩进 selection 与自动缩进均经 adapter 访问私有 `.vditor-sv`、source line/Range、list marker/padding 和 capture `keydown` 路径。Vditor 3.11.3 的 `Ctrl/Cmd+Shift+I` 为减少缩进、`Ctrl/Cmd+Shift+O` 为增加缩进；它会在 SV 中禁用自己的 toolbar action，adapter 必须在 source 选区上执行同一语义。空白符 canvas 在重绘时以当前 viewport 坐标重新生成并归零旧滚动补偿 transform。Desktop 的 source → preview 同步按 source heading marker 与 preview 直接 H1–H6 的等量顺序配对，在相邻锚点间插值，并以两侧 viewport 高度的 20% 作为标题对齐基准；集合不等或缺少标题时必须保留 Vditor 原生高度比例同步，preview 滚动不得回写 source。升级时须验证长文档、空行、表格/HTML 源码、空白符开关、source 滚动、标题锚点同步与不匹配回退、preview 独立阅读、工具栏和两条快捷键的缩进/反缩进、非列表行 Enter 自动缩进；pane 替换、rebuild 或关闭后不得遗留 observer、rAF、scroll 或 keydown listener。
...
- Desktop 自绘光标通过 adapter 的 `installCustomCaret()` 读取三种模式的私有可编辑根和折叠 Range 几何；浏览器原生 Selection/Range 仍是唯一真实插入点。代理节点挂在 `#editorArea` 内以共享其 `overflow: hidden` 裁剪，滚动时先按 `scrollTop`/`scrollLeft` 增量同步平移、下一帧再用真实 Range 几何校正并重启闪烁，光标越出编辑面或 sidebar 拖动期间隐藏。表格内光标以编辑器根的行高计算几何并在越出表格可视区域时隐藏；Vditor 3.11.3 在 WYSIWYG/IR 完成行内 Markdown 格式化后可能把折叠 Range 放在段落子节点之后，浏览器返回空矩形，adapter 必须用前一行内节点的末端定位，不能取整个段落的左边缘。IR 空块之间的折叠选区可报告零矩形，adapter 会瞬时插入零宽探针节点取得插入行几何后立即移除，不回写内容、不派发 input。IR 中 Vditor 3.11.3 把撤销后的折叠 Range 恢复到未展开标题 `.vditor-ir__marker--heading` 文本或其后元素边界时，adapter 与 Chromium 原生行为一致地不绘制代理，标题带 `vditor-ir__node--expand` 展开后恢复正常绘制。`caretStyle` 为 `native` 时组合层仅挂起代理（`suspendCustomCaret()`），沿用 Chromium/Vditor 原生 caret，`underline`/`bar`/`block` 才安装代理。升级时须验证普通文本、空段落、折行、WYSIWYG/IR 输入粗体与行内代码结束标记后的定位、IR 空块、表格和 SV 中的定位，并确认失焦、选区、IME、不可见窗口、rebuild 和 tab close 不保留代理节点、探针节点或隐藏原生光标。
