...
- Desktop 自绘光标通过 adapter 的 `installCustomCaret()` 读取三种模式的私有可编辑根和折叠 Range 几何；浏览器原生 Selection/Range 仍是唯一真实插入点。代理节点挂在 `#editorArea` 内以共享其 `overflow: hidden` 裁剪，滚动时先按 `scrollTop`/`scrollLeft` 增量同步平移、下一帧再用真实 Range 几何校正并重启闪烁，光标越出编辑面或 sidebar 拖动期间隐藏。表格内光标以编辑器根的行高计算几何并在越出表格可视区域时隐藏；IR 空块之间的折叠选区可报告零矩形，adapter 会瞬时插入零宽探针节点取得插入行几何后立即移除，不回写内容、不派发 input。IR 中 Vditor 3.11.3 把撤销后的折叠 Range 恢复到未展开标题 `.vditor-ir__marker--heading` 文本或其后元素边界时，adapter 与 Chromium 原生行为一致地不绘制代理，标题带 `vditor-ir__node--expand` 展开后恢复正常绘制。`caretStyle` 为 `native` 时组合层仅挂起代理（`suspendCustomCaret()`），沿用 Chromium/Vditor 原生 caret，`underline`/`bar`/`block` 才安装代理。升级时须验证普通文本、空段落、折行、IR 空块、表格和 SV 中的定位，并确认失焦、选区、IME、不可见窗口、rebuild 和 tab close 不保留代理节点、探针节点或隐藏原生光标。
