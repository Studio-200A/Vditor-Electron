...
Vditor Desktop 不修改 `node_modules/vditor` 的源码，但工具栏合并、SV 行号与空白字符、列表缩进和大纲跳转依赖 Vditor 的内部 DOM。升级必须作为显式维护任务进行，不能随普通依赖更新自动合入。
...
6. 手工验证三种编辑模式、统一工具栏、主题菜单、列表缩进、SV 行号/灰点/滚动和大纲跳转。
