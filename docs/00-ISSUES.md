...
- 批次 4 的外部修改冲突、批次 5 的文件删除/重新出现/不可读状态、批次 5.1 的工作区读取/监听深度边界、批次 6 的目录级路径一致性、批次 7 的本地闭环以及批次 8/9/9.1 的安全代码、专项验证、Linux 全量回归和手测均已完成。批次 10/11 已完成本地实现、专项自动化和手测闭环；2026-08-31 用户全量检查的静态阶段和 181/181 单测通过，129 条 E2E 中 1 条既有时序用例首次失败后以原用例精确复跑通过。当前限制集中在跨应用重启后的外部状态恢复、本地资源安全、已有目标的长期 TOCTOU、主窗口 sandbox preload 迁移和跨平台实体机验证。长期文件安全契约见 [`docs/05-FILE-SAFETY.md`](05-FILE-SAFETY.md)。
