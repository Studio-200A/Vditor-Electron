...
0.2.5 的完整开发和实机验收继续以 Linux 为准，Windows/macOS 的实体机回归留待后续专用分支。重构不得因此把 Linux 假设固化到 renderer。

- 平台分支优先保留在 main process 的路径、文件服务和窗口集成边界；renderer 控制器应消费窄 bridge 提供的能力或结果，而不是反复判断操作系统名称。
- 如标题栏、窗口控制、快捷键文案或布局确实需要平台差异，bridge 应提供最小能力契约，例如主修饰键、是否使用自绘窗口控制和标题栏安全区域；不要建立泛化 `PlatformService`。
- 文件路径仍作为不透明的原生路径值经 bridge 传递。路径展示、资源 URL 和比较必须通过明确函数处理，不能假定 POSIX 分隔符或 Linux 大小写语义。
- Linux 上的单元和 fake bridge 测试应能构造 Windows/macOS 能力与路径输入；这只用于防止架构回归，不替代未来真实平台测试。
...
与 renderer 并列建立 `src/shared/contracts/`，仅放置主进程、preload 与 renderer 共同使用的可序列化 IPC 请求、响应、错误结果和设置契约。这里不得引入 Electron、Node、DOM、Vditor 实例或其他运行时依赖；renderer 内部状态与控制器类型仍保留在 `src/renderer/types/`。
...
Store 可以提供窄粒度订阅，例如 `subscribe(selector, listener)`，供确实需要广播的派生 UI 使用；订阅必须返回 unsubscribe，并由控制器纳入 dispose。不得把所有 `updateSettings()` 自动广播给 ThemeController、EditorController 等模块：主题、字体、缩放和编辑器重建的影响范围不同，实际应用应由明确的 Controller 或组合层协调，避免隐式级联和不必要的 Vditor 重建。
