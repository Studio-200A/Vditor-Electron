...
1. `0.2.0`：优先完成编辑器基础可靠性、文件冲突处理、安全边界和发布验证；当前 tracker 批次 14 已完成实现与专项验证，批次 15 已完成版本、Electron、Node 和项目 metadata 的本地实现，用户全量检查、批次 14 最终复验、Linux 候选包和跨平台验证仍按各自文档推进；暂不推进浮动工具栏方案。
