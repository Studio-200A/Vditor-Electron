...
- 长文档也不会迷路：实时大纲支持跳转到任何 H1–H6 标题（所见即所得、即时渲染，或已开启预览的分栏预览），与桌面 UI 自然融合。
...
- **重建文件也留下退路：** 只有在你明确确认后，软件才会重建缺失文件，并把文件变得不可访问那一刻捕获的内容复制到系统剪贴板，同时显示 5 秒确认提示。提示出现后继续输入的内容不会混入这份备份，因此这份备份正是你可能需要的那份受保护版本。
...
- **Nord 深色**，映射自 [Nord 官方 color palette](https://github.com/nordtheme/nord/blob/develop/src/nord.css)，包含独立的 H1–H6 标题配色
...
| 新建文件                       | `Ctrl/Cmd + N`         |
| 打开文件                       | `Ctrl/Cmd + Alt + O`   |
| 打开文件夹                     | `Ctrl/Cmd + Alt + K`   |
| 保存                           | `Ctrl/Cmd + S`         |
| 另存为                         | `Ctrl/Cmd + Shift + S` |
| 查找和替换                     | `Ctrl/Cmd + F`         |
| 选择当前上下文 / 全文          | `Ctrl/Cmd + A`         |
| 关闭标签页                     | `Ctrl/Cmd + W`         |
| 切换资源管理器                 | `Ctrl/Cmd + Alt + B`   |
| 打开设置                       | `Ctrl/Cmd + ,`         |
| 切换 Chrome DevTools（启用后） | `F12`                  |
| 切换全屏                       | `F11`                  |
