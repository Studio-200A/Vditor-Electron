...
主题切换会记住用户上一次使用的深色主题。内容主题和代码块预览主题会跟随应用的浅色/深色模式，同时分别保留用户在这两种模式下最后选择的主题。需要时可以启用多平台排版预览。
...
“文件与会话”提供“**工作区目录最大读取深度**”设置，默认 7 层，可选 7–12 层。它同时限制文件树读取与工作区监听，帮助大型目录保持流畅；需要访问更深层内容时，建议将更窄的目录直接设为工作区。

文件树会识别真实路径仍位于工作区内的目录链接，并用视觉标记区分，同时遵守相同的深度限制；指向工作区外的链接仍会显示，但不可展开。Windows `.lnk` 快捷方式和 macOS Finder alias 不属于目录链接，目前仍按普通文件处理。
