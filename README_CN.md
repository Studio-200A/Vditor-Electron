...
- **清晰的菜单交互。** 打开 sidebar 或编辑区右键菜单时，会自动收回正在显示的主菜单。
...
Vditor Desktop 把你的写作当作需要保护的内容，而不是可以随时覆盖的数据。在简洁的 Markdown 工作流背后，我们加入了多重保护设计，尽量避免异常退出、其他软件修改文件或文件系统变化时，你的内容被静默带走：

- **更谨慎地保存。** 保存时会先在同目录写入临时文件；内容未变化时不会重写，保存失败也会保留原文件和编辑器中的未保存内容。
...
- **引导解决内容冲突。** 你可以重载稳定的磁盘版本、将当前内容另存为、保留为未命名文档、忽略外部变化，或经过明确确认后覆盖磁盘版本。如果磁盘再次发生变化，之前的覆盖确认会自动失效。
- **防止文件消失或失去权限。** 文件被删除或暂时无法读写时，编辑器会保留内存中的内容并暂停自动保存，不会偷偷重建或覆盖文件。文件恢复访问后，也不会未经决定就把磁盘版本塞回编辑器。
- **重建文件也留下退路。** 只有在你明确确认后，软件才会重建缺失文件，并把文件变得不可访问那一刻捕获的内容复制到系统剪切板，同时显示 5 秒确认提示。提示出现后继续输入的内容不会混入这份备份。
...
目前 Linux 是主要开发和验证平台；项目已经包含 Windows 和 macOS 的窗口及数据目录适配，但 watcher、权限、路径大小写、打包和发布仍需在实体设备上验证。
