...
- 监控所有已打开文件的变化，包括工作区外独立打开的文件；没有本地修改的文档会自动重载，有本地修改的文档会显示持久冲突横幅，并在处理前暂停自动保存。
- 冲突横幅可重载最新稳定磁盘版本、将当前内容另存为、忽略外部更改，或请求经过明确确认的覆盖；磁盘再次变化时旧的覆盖确认会失效。
- 粘贴或上传图片到可配置的相对资源目录；相对路径图片和在线图片可以在三种模式中预览。

当前外部文件监控覆盖所有已打开文件，包括活动工作区外独立打开的文件。文件删除后的恢复、目录移动和跨应用重启的冲突恢复仍属于后续可靠性工作；重要文档请保留备份。
